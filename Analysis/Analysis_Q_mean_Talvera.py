...
# QUERY DATA BASE
query = 'SELECT Timestamp, Q_mean from Tabella_Talvera' 
df = pd.read_sql(query, con=connection)
print(df)
# CLEAN THE DATA
droppare = []
i = 0 
while i < (len(df['Q_mean'])):
        if np.isnan(df['Q_mean'].iloc[i]):
                droppare.append(i)
        i += 1
...
plt.plot(y_21['Q_mean'], label = '2021')
plt.plot(y_20['Q_mean'], color = 'green', label= '2020')
plt.plot(y_19['Q_mean'], color = 'red', label = '2019' )
plt.xlabel('Date')
plt.ylabel('Water Level')
plt.title('Water level Talvera')
plt.legend()
plt.xticks()
# MODIFICARE I TICKS 
plt.show()
...
print(df.loc[df["Q_mean"] == df["Q_mean"].max()])
print(df.loc[df["Q_mean"] == df["Q_mean"].min()])

# Plot the daily temperature change 
plt.figure(figsize=(16,10), dpi=100)
plt.plot(df.index, df['Q_mean'], color='tab:red')
# MODIFICARE I TICKS 
plt.gca().set(title="Water level Talvera", xlabel='Date', ylabel="Water Level")
plt.show()

from statsmodels.tsa.seasonal import seasonal_decompose

# Additive Decomposition
result_add = seasonal_decompose(df.Q_mean, model='additive', extrapolate_trend='freq', freq=365)
...
predicted_df = df["Q_mean"].to_frame().shift(1).rename(columns = {"Q_mean": "Q_mean_pred" })
actual_df = df["Q_mean"].to_frame().rename(columns = {"Q_mean": "Q_mean_actual" })
...
# Calculate the RMSE
temp_pred_err = MSE(one_step_df.Q_mean_actual, one_step_df.Q_mean_pred, squared=False)
...
for param in pdq:
    for param_seasonal in seasonal_pdq:
        try:
            mod = sm.tsa.statespace.SARIMAX(one_step_df.Q_mean_actual,
                                            order=param,
...
for pred_time in pred_list:
    ...
    output_l = str(pred_ci['lower Q_mean_actual']).split()
    output_u = str(pred_ci['upper Q_mean_actual']).split()
    output = output_l[1] + ' - ' + output_u[1]
    list_output.append(output)

    plt.figure(figsize=(16,10), dpi=100)
    ax = one_step_df.Q_mean_actual[:].plot(label='observed')
