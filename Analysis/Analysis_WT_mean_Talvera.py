...
plt.figure(figsize=(16,10), dpi=100)
plt.plot(df.index, df['WT_mean'], color='tab:red')
# MODIFICARE I TICKS 
...
result_add = seasonal_decompose(df.WT_mean, model='additive', extrapolate_trend='freq', freq=365)
...
predicted_df = df["WT_mean"].to_frame().shift(1).rename(columns = {"WT_mean": "WT_mean_pred" })
actual_df = df["WT_mean"].to_frame().rename(columns = {"WT_mean": "WT_mean_actual" })
...
# Calculate the RMSE
temp_pred_err = MSE(one_step_df.WT_mean_actual, one_step_df.WT_mean_pred, squared=False)
...
for param in pdq:
    for param_seasonal in seasonal_pdq:
        try:
            mod = sm.tsa.statespace.SARIMAX(one_step_df.WT_mean_actual,
                                            order=param,
...
for pred_time in pred_list:
    ...
    output_l = str(pred_ci['lower WT_mean_actual']).split()
    output_u = str(pred_ci['upper WT_mean_actual']).split()
    output = output_l[1] + ' - ' + output_u[1]
    list_output.append(output)

    plt.figure(figsize=(16,10), dpi=100)
    ax = one_step_df.WT_mean_actual[:].plot(label='observed')
